...
def newGeneration(old_population):
    ...
    for n, agent in enumerate(old_population):
        ...
        #fitness[n] = agent.turn / 90 + 5*agent.alive + 2 * agent.size + agent.strawb_eats + agent.enemy_eats * 3
        fitness[n] = (agent.alive + agent.turn/90) * (agent.size + 1)* (agent.size + 1) * (agent.enemy_eats)
