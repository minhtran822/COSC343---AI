...
class Player:
    ...
    def new_generation_agents(self):
        ...
        if (len(self.fitness) == 500):
            print("Min: %.2f" % np.amin(self.fitness))
            print("Max: %.2f" % np.amax(self.fitness))
            print("First: %.2f" % self.fitness[0])
            print("Last: %.2f" % self.fitness[499])
    ...
    def run(self,player1File, player2File,show_games, save_games, visResolution=(720,480), visSpeed='normal',savePath="saved"):
        ...
        for game in range(1,self.nGames+1):
            ...
            if survivorCount[0]>survivorCount[1]:
                ...
                if(survivorCount[0] <= 30 or survivorCount[1] >= 5):
                    bluecount += 1
            ...
            if(game % 100 ==0):
                bluenum.append(bluecount)
                bluecount = 0
                rednum.append(redcount)
                redcount = 0
